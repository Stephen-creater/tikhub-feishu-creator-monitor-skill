...
def normalize_profile(payload: dict, *, fetched_at: datetime) -> Account:
    ...
    account_id = str(first(user, ("userid", "userId", "user_id", "id")))
    return Account(
        ...
        works_count=parse_count(
            first(
                data,
                ("noteCount", "note_count", "notes", "ndiscovery"),
                first(as_dict(data.get("note_num_stat")), ("posted",)),
            )
        ),
...
def normalize_notes(payload: dict, *, fetched_at: datetime) -> tuple[list[Content], PageInfo]:
    ...
    for value in raw_notes:
        ...
        user = as_dict(first(item, ("user", "userInfo", "user_info"), {}))
        cover = first(item, ("cover", "coverInfo", "imageList", "images_list"))
        video = as_dict(first(item, ("video", "videoInfo", "video_info_v2"), {}))
        video_image = as_dict(video.get("image"))
        video_media = as_dict(video.get("media"))
        stream = as_dict(video_media.get("stream"))
        streams = as_list(stream.get("h264")) or as_list(stream.get("h265"))
        records.append(
            Content(
                platform=Platform.XIAOHONGSHU,
                content_id=note_id,
                account_id=str(
                    first(
                        item,
                        ("userId", "user_id", "authorId"),
                        first(user, ("userid", "userId", "user_id", "id")),
                    )
                ),
                ...
                cover_url=first_url(cover) or first_url(video_image),
                media_url=first_url(first(item, ("videoUrl", "video_url")))
                or first_url(streams),
                ...
                likes=parse_count(
                    first(stats, ("likedCount", "likeCount", "likes"), item.get("likes"))
                ),
                comments=parse_count(
                    first(
                        stats,
                        ("commentCount", "comments"),
                        first(item, ("comments_count", "comment_count")),
                    )
                ),
                shares=parse_count(
                    first(
                        stats,
                        ("shareCount", "shares"),
                        first(item, ("share_count", "shares_count")),
                    )
                ),
                saves=parse_count(
                    first(
                        stats,
                        ("collectedCount", "collectCount", "saves"),
                        first(item, ("collected_count", "collect_count")),
                    )
                ),
    ...
    if cursor in (None, "") and raw_notes:
        cursor = first(as_dict(raw_notes[-1]), ("cursor", "noteId", "note_id", "id"))
